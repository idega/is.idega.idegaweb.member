...
public class GroupApplicationOverView extends Page implements IWBrowserView {
	...
	public Table getApplications(IWContext iwc, String status) throws Exception {
		...
		/*
		 * if( selectedGroup == null ){ selectedGroup= ps.getSelectedGroup(); }
		 */
	...
	/*
	 * public IWPresentationState getPresentationState(IWUserContext iwuc){ if(_presentationState == null){ try { IWStateMachine stateMachine =
	 * (IWStateMachine)IBOLookup.getSessionInstance(iwuc,IWStateMachine.class); _presentationState =
	 * (GroupApplicationOverViewPS)stateMachine.getStateFor(this.getLocation(),this.getPresentationStateClass()); } catch (RemoteException re) { throw
	 * new RuntimeException(re.getMessage()); } } return _presentationState; }
	 * 
	 * public Class getPresentationStateClass(){ return GroupApplicationOverViewPS.class; }
	 */
